...
⚠️  Если каналы кампании отображаются неправильно

Некоторые настройки Discord могут скрывать новые каналы или помещать их в раздел рекомендаций.

Чтобы вернуть нормальное отображение:

1. Нажмите ПКМ по названию сервера.
2. Выберите **«Показать все каналы» (Show All Channels)**.

После этого категория кампании и все её каналы должны отображаться корректно.

Данная проблема связана с настройками Discord и не является ошибкой бота.

При необходимости уведомьте своих игроков.
`,
    components: [
      {
        type: 1,
